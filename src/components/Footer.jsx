...
const Footer = () => {
  ...
  return (
    <footer className="bg-gray-900 text-gray-300">
      {/* Newsletter Section */}
      <div className="bg-yellow-500 text-gray-900 py-8">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-bold">Get Exclusive Deals!</h3>
              <p className="text-gray-800">Subscribe for latest products and special offers</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-white text-xl font-bold">Electro.pk</span>
            </div>
          ...
          {/* Categories */}
          <div>
            <h4 className="text-white text-lg font-semibold mb-4">Product Categories</h4>
            <ul className="space-y-2">
              {categories.map((cat) => (
                <li key={cat.path}>
                  <Link to={cat.path} className="hover:text-yellow-500 transition-colors">
                    {cat.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                <span>📍</span>
                <span>Shop #123, Main Market, Gulberg III, Lahore, Pakistan</span>
              </li>
              <li className="flex items-center gap-3">
                <span>📞</span>
                <a href="tel:+923211234567" className="hover:text-yellow-500">+92 321 1234567</a>
              </li>
              <li className="flex items-center gap-3">
                <span>📧</span>
                <a href="mailto:info@electro.pk" className="hover:text-yellow-500">info@electro.pk</a>
              </li>
              <li className="flex items-center gap-3">
                <span>⏰</span>
            ...
            {/* Payment Methods */}
            <div className="mt-6">
              <h5 className="text-white font-medium mb-3">We Accept:</h5>
              <div className="flex flex-wrap gap-2">
                {paymentMethods.map((pm) => (
                  <span key={pm.name} className="bg-gray-800 px-3 py-1 rounded-full text-sm">
                    {pm.icon} {pm.name}
                  </span>
                ))}
              </div>
        ...
        <div className="border-t border-gray-800 mt-8 pt-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-gray-400 text-sm">
              © {currentYear} Electro.pk. All rights reserved. Made with ❤️ in Pakistan
            </p>
            <div className="flex items-center gap-6 text-sm">
              ...
              <Link to="/refund" className="hover:text-yellow-500">Refund Policy</Link>
            </div>
          </div>
        </div>
      </div>
